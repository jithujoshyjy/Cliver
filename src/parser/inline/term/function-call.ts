...
import { skip, _skipables, createMismatchToken, isPunctuator, isBlockedType, withBlocked, withUnblocked } from "../../utility.js"
...
export function generateFunctionCall(context: string[], tokens: TokenStream): FunctionCall | MismatchToken {
	...
	const nodeGenerators = [
		generateTaggedSymbol, generateTaggedString, generatePropertyAccess, generateDoExpr, generateTaggedNumber, generateImplicitMultiplication,
	...
	for (const nodeGenerator of nodeGenerators) {
		...
		if (isBlockedType(nodeGenerator.name.replace("generate", ""), ["PropertyAccess"]))
			continue

		caller = withBlocked(["FunctionCall"], () => nodeGenerator(["FunctionCall", ...context], tokens))

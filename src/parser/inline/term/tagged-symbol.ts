...
import { createMismatchToken, skip, _skipables, NodePrinter, pickPrinter, isBlockedType, withBlocked } from "../../utility.js"
...
export function generateTaggedSymbol(context: string[], tokens: TokenStream): TaggedSymbol | MismatchToken {
	...
	const nodeGenerators = [
		generateTaggedString, generateEitherPropertyAccessOrFunctionCall, generateIdentifier, generateGroupExpression
	]
	...
	for (const nodeGenerator of nodeGenerators) {
		...
		tag = withBlocked(["TaggedSymbol"],
			() => nodeGenerator(["TaggedSymbol", ...context], tokens))
